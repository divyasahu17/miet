...
import { isConsultantApproved } from '@/utils/consultant';
...
export default function SearchPanel() {
  ...
  const handleLoginSuccess = (userData: any) => {
    ...
    // Check if we have a pending booking in state or localStorage
    const pendingId = pendingBookingConsultant?.id || localStorage.getItem('pending_consultant_id');
    
    if (pendingId) {
      window.location.href = `/${locale}/services/consultations?consultantId=${pendingId}`;
    } else {
  ...
  const handleBookClick = (consultant: Consultant) => {
    if (user) {
      // Redirect to consultations page with specific consultant if user is logged in
      window.location.href = `/${locale}/services/consultations?consultantId=${consultant.id}`;
    } else {
      localStorage.setItem('pending_consultant_id', consultant.id.toString());
      localStorage.setItem('pending_consultant_name', consultant.name);
    ...
    <section className="search-map-panel" style={{
                      ...
                      <div style={{ minWidth: 280, maxWidth: 320, padding: 8, position: 'relative' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                          ...
                          <div style={{ marginTop: 12, borderTop: '1px solid var(--border)', paddingTop: 12 }}>
                            <div style={{ color: 'var(--text-accent-alt)', fontSize: 13, fontWeight: 700, marginBottom: 4 }}>Platform Support</div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                              <a href="mailto:miet.life@gmail.com" style={{ color: 'var(--accent)', fontSize: 12, display: 'flex', alignItems: 'center', gap: 6, textDecoration: 'none' }}>
                                ✉️ miet.life@gmail.com
                              </a>
                              <a href="tel:+919319027664" style={{ color: 'var(--accent)', fontSize: 12, display: 'flex', alignItems: 'center', gap: 6, textDecoration: 'none' }}>
                                📞 +91 9319027664
                              </a>
                            </div>
                          </div>
      ...
      {/* Booking Modal */}
      {
        bookingConsultant && (() => {
          ...
          return (
            <div role="dialog" aria-modal="true" tabIndex={-1} style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(34,37,77,0.32)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={e => { if (e.target === e.currentTarget) setBookingConsultant(null); }}>
