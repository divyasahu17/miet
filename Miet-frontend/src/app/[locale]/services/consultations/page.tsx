...
export default function ConsultationsPage() {
  ...
  // Check authentication status and URL params on component mount
  useEffect(() => {
    checkAuthStatus();
    
    // Check for consultantId in URL
    const params = new URLSearchParams(window.location.search);
    const consultantId = params.get('consultantId');
    if (consultantId) {
      localStorage.setItem('pending_consultant_id', consultantId);
    }
  }, []);
  ...
  const loadConsultants = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setConsultants((data || []).filter(isConsultantApproved));
      }
    } catch (error) {
  ...
  // Auto-open booking modal if we have a pending consultant ID
  useEffect(() => {
    if (!loading && consultants.length > 0) {
      const pendingId = localStorage.getItem('pending_consultant_id');
      if (pendingId) {
        const consultant = consultants.find(c => c.id.toString() === pendingId);
        if (consultant) {
          handleBookClick(consultant);
          // Clear it so it doesn't pop up again on refresh
          localStorage.removeItem('pending_consultant_id');
          localStorage.removeItem('pending_consultant_name');
          
          // Clear URL param without refreshing
          const url = new URL(window.location.href);
          url.searchParams.delete('consultantId');
