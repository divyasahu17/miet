...
import { FaSave, FaSpinner, FaShieldAlt } from 'react-icons/fa';
...
export default function PrivacyTab() {
  ...
  const fetchCmsContent = async () => {
    ...
    try {
      ...
      const res = await fetch(getApiUrl('api/cms?page_key=privacy'), {
      ...
      if (res.ok) {
        ...
        const privacyFields = raw.filter((item: any) => item.section_key === 'content');
        
        const newFormData = { ...formData };
        privacyFields.forEach((item: any) => {
      ...
      console.error('Error fetching CMS privacy:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{
        ...
        <div>
          <h2 style={{
            ...
            <FaShieldAlt style={{ color: '#667eea' }} /> Privacy Policy
          </h2>
          <p style={{ margin: '6px 0 0 0', color: '#666', fontSize: '14px' }}>Manage your site's Privacy Policy content.</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <form onSubmit={handleSave} style={{
          ...
          <div>
            <label style={{ display: 'block', marginBottom: 8, fontWeight: 600, color: '#374151' }}>Policy Content (HTML Supported)</label>
            <textarea name="body" value={formData.body} onChange={handleChange} rows={15} style={textareaStyle} placeholder="<p>Enter your privacy policy here...</p>" />
          </div>
